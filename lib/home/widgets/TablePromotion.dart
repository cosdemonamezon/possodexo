...
class _TablePromotionState extends State<TablePromotion> {
  ...
  List<DataRow> getRows(List<ProDetail> prodetails) =>
      prodetails.map((ProDetail detail) {
        final cells = [
          detail.promoCode,
          detail.type,
  ...
  void onSort(int columIndex, bool ascending) {
    if (columIndex == 0) {
      prodetails.sort((detail1, detail2) =>
          compareString(ascending, detail1.promoCode, detail2.promoCode));
    } else if (columIndex == 1) {
      prodetails.sort((detail1, detail2) =>
          compareString(ascending, detail1.type, detail2.type));
    } else if (columIndex == 2) {
      prodetails.sort((detail1, detail2) => compareString(
          ascending, detail1.promotionName, detail2.promotionName));
    } else if (columIndex == 3) {
      prodetails.sort((detail1, detail2) =>
          compareString(ascending, detail1.details, detail2.details));
    } else if (columIndex == 4) {
      prodetails.sort((detail1, detail2) =>
          compareString(ascending, detail1.StartDate, detail2.StartDate));
    } else if (columIndex == 5) {
      prodetails.sort((detail1, detail2) => compareString(
          ascending, detail1.ExpirationDate, detail2.ExpirationDate));
    }
    setState(() {
  ...
  int compareString(bool ascending, String detail1, String detail2) =>
      ascending ? detail1.compareTo(detail2) : detail2.compareTo(detail1);
}
